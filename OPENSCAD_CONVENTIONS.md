...
코드 바로 옆 주석은 `대상 용어(English term) — 역할/이유` 형식으로 쓴다. 앞의 **대상 용어는 짧은 구(phrase)**로, 뒤의 **역할/이유는 동사로 끝나는 절(clause)**로 쓴다(역할/이유를 명사구로 끊지 않는다). 기계적 역할과 제약이 한눈에 읽히도록 하고, `공칭 지름(nominal diameter, 호칭 치수)`처럼 오해될 수 있는 전문어는 괄호 안에 영어와 짧은 해석을 함께 붙인다.

```scad
// 외륜 숄더(outer-race shoulder) — 내륜(inner race) 바깥에서만 축방향으로 받쳐, 회전하는 내륜과 실드를 건드리지 않는다.
circle(d = inner_race_outside_diameter + bearing_clearance);
```

대상 용어가 자명하면 `—` 없이 역할/이유 절만 써도 된다. 단, 파일 헤더처럼 독립 문맥이 필요한 주석이나 복잡한 설계 의사결정은 C-1에 따라 여러 문장으로 이유를 설명할 수 있다.
...
모든 라벨 모듈은 `$preview` 가드 안에서만 그리고, 글자는 얇은 `linear_extrude`로 만든다. STL·절단·간섭 형상에는 들어가지 않는다.

### A-2 (SHOULD) 라벨 위치는 고정값이 아니라 뷰포트와 부품 좌표로 계산한다

지시선 lead를 손으로 박지 않는다. 부품 점과 뷰포트 회전(`$vpr`)에서 화면축(화면 우/상/법선)을 유도해 라벨을 화면 좌/우 열에 자동 분배하고, 열 안에서 균등 간격으로 펼쳐 겹치거나 가리지 않게 한다(`callout_field`). 카메라를 돌리면 라벨도 따라 재배치된다.
...
부품이 받는 분해 변환(거리비례 bloom + explode Z)을 라벨 점에도 그대로 적용한다(`label_point`). 분해해도 라벨이 부품에 붙어 있어야 한다.

### A-5 (SHOULD) 라벨은 종류 토글로 끄고 켠다

기능군으로 토글을 나눈다 — 체결구 / 구조·개념(관절·축·링크) / 동적 기성품(모터·풀리·벨트·베어링·이송나사). 체결구는 라벨만 보고 조립할 수 있을 만큼 위치·역할까지 빠짐없이 달고(중복 허용), 구조는 J1/J2 같은 관절·축과 링크 개념을 빠짐없이 단다. 데이터 스키마는 `[텍스트, 점, 그룹, 강조, bloom, ez, 색]`을 쓴다.
...
- [ ] 조립 라벨이 `$preview` 전용이고, 색은 대상과 같으며, exploded에서 함께 움직이는가? (A-1, A-3, A-4)
