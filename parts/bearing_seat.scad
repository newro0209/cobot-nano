// parts/bearing_seat.scad - Bearing-seat geometry cut into / added to sheet plates (NopSCADlib printed-part style).
...
function bearing_shield_thickness(type) = bb_diameter(type) - bb_bore(type) - bb_rim(type) - bb_hub(type);
...
module bearing_seat_pocket(type, bore_depth, from_top = false) {
    ...
    rim_grip_diameter = bore + hub + bearing_shield_thickness(type) / 2;

    // 관통 보어(through bore) — 부품 전체 깊이를 림 무는 지름으로 관통한다.
    translate([0, 0, -eps])
        cylinder(h = bore_depth + eps * 2, d = rim_grip_diameter + shaft_clearance);

    // 안착 카운터보어(counterbore) — from_top이면 윗면, 아니면 바닥면으로 열려 베어링 삽입 면을 고른다.
    seat_z = from_top ? bore_depth - width : -eps;
...
module bearing_inner_race_boss(type, height = seat_shoulder_thickness) {
...
translate([30, 0, 0])
    difference() {
        ...
        bearing_seat_pocket(BB608, bore_depth = 10);
    }
