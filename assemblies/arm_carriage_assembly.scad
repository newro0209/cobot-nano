...
// J2 숄더 볼트: 숄더부(shoulder)가 상판 윗면부터 하판 아랫면까지 스택 전체를 관통하고, 그 아래 나사산에 락너트가 물린다.
// 머리 와셔 두께만큼 더해 나사산이 하판 아랫면에서 시작하도록 한다(screw()는 숄더 길이를 length로 받고 나사산은 그 아래에 붙인다).
...
// J2 종동축 숄더 볼트 — 상판 윗면에서 와셔를 깔고, 숄더부가 두 BB608 보어를 채우며 스택을 관통해 내려간다(상판 그룹).
...
// J2 종동축 락너트 — 하판 아랫면으로 빠져나온 나사산에 와셔+나일록 너트로 스택을 축방향 고정한다(하판 그룹).
